...
void publish(long tmp_unixTime, short tmp_temp, short tmp_hum,
  ...
  leaf2["ts"] = ut;
  leaf2["meaning"] = "weight_raw";
  leaf2["value"] = tmp_weight;

  //  Creating MQTT packet
  char message_buff[MQTT_MAX_PACKET_SIZE];
  root.printTo(message_buff, sizeof(message_buff));

  char mess_new[MQTT_MAX_PACKET_SIZE];

  //  Mapping the JSON string and removing the " " from the timestamp field
  //  since it was treated as a string
  int idxToDel = 7;
  memmove(&message_buff[idxToDel], &message_buff[idxToDel + 1],
          strlen(message_buff) - idxToDel);
  idxToDel = 20;
  memmove(&message_buff[idxToDel], &message_buff[idxToDel + 1],
          strlen(message_buff) - idxToDel);
  idxToDel = 69;
...
void runServer() {
  ...
  while (!wifiClient.available()) {
    delay(1000);
    Serial.print("Server not running yet");
  }
...
void getLoadReadings() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Serial.println(String(loadArray[i]) + " " + String(tareArray[i]) + " " +
    // String(loadArray[i] - tareArray[i]));

    sum += (loadArray[i] - tareArray[i]);
  }
  // Serial.println(sum);
  weight_tmp = calibrate(sum / 4);
